...
export default function ReportesPage() {
  ...
  function getPLData() {
    const filtered = transactions.filter(
      (t) => new Date(t.date).getFullYear() === year && t.status !== "ANULADO",
    );
    ...
    const totalIVACobrado = ingresos.reduce(
      (s, t) => s + Number(t.taxAmount || 0),
      0,
    );
    const totalIVAPagado = gastos.reduce(
      (s, t) => s + Number(t.taxAmount || 0),
      0,
    );
    ...
    gastos.forEach((t) => {
      gastosByCategory[t.category] =
        (gastosByCategory[t.category] || 0) + Number(t.amount);
    });
    const ingresosByCategory: Record<string, number> = {};
    ingresos.forEach((t) => {
      ingresosByCategory[t.category] =
        (ingresosByCategory[t.category] || 0) + Number(t.amount);
    });
  ...
  function exportCSV() {
    ...
    if (reportType === "pl") {
      ...
      Object.entries(pl.ingresosByCategory).forEach(([cat, amt]) => {
        csv += `  ${cat},${amt}\n`;
      });
      csv += `Total Gastos,${pl.totalGastos}\n`;
      Object.entries(pl.gastosByCategory).forEach(([cat, amt]) => {
        csv += `  ${cat},${amt}\n`;
      });
      ...
      getMonthlyData().forEach((m) => {
        csv += `${m.month},${m.ingresos},${m.gastos},${m.balance},${m.count}\n`;
      });
    } else {
      const filtered = transactions.filter(
      ...
      filtered.forEach((t) => {
        csv += `${t.date},${t.type},${t.category},"${t.description}",${t.amount},${t.taxAmount || 0},${t.status}\n`;
      });
    }
    ...
    a.href = url;
    a.download = `reporte_${reportType}_${year}.csv`;
    a.click();
              ...
              >
                <tbody>
                  <tr style={{ borderBottom: "2px solid var(--color-border)" }}>
                    ...
                    >
                      {formatCurrency(pl.totalIngresos)}
                    </td>
                  </tr>
                  {Object.entries(pl.ingresosByCategory)
                    .sort((a, b) => b[1] - a[1])
                    .map(([cat, amt]) => (
                      <tr
                        key={cat}
                  ...
                  >
                    ...
                    >
                      {formatCurrency(pl.totalGastos)}
                    </td>
                  </tr>
                  {Object.entries(pl.gastosByCategory)
                    .sort((a, b) => b[1] - a[1])
                    .map(([cat, amt]) => (
                      <tr
                        key={cat}
                  ...
                  >
                    ...
                    <td
                      style={{
                        ...
                        textAlign: "right",
                        color:
                          pl.totalIngresos - pl.totalGastos >= 0
                            ? "#00e676"
                            : "#ff5252",
                      }}
                    >
          ...
          {reportType === "monthly" && (
            <div className="glass-card" style={{ padding: "var(--space-6)" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ borderBottom: "2px solid var(--color-border)" }}>
                    {["Mes", "Ingresos", "Gastos", "Balance", "Txs"].map(
                      (h) => (
                        <th
                          key={h}
                          style={{
                ...
                <tbody>
                    ...
                    >
                      ...
                      <td
                        style={{
                          padding: "var(--space-3)",
                          textAlign: "right",
                          color: m.balance >= 0 ? "#00e676" : "#ff5252",
                          fontWeight: 700,
                          fontSize: "var(--text-sm)",
                        }}
                      >
                  ...
                  >
                    ...
                    >
                      {formatCurrency(
                        monthly.reduce((s, m) => s + m.ingresos, 0),
                      )}
                    </td>
                    <td
                      style={{
                    ...
                    >
                      {formatCurrency(
                        monthly.reduce((s, m) => s + m.gastos, 0),
                      )}
                    </td>
                    <td
                      style={{
                    ...
                    >
                      {formatCurrency(
                        monthly.reduce((s, m) => s + m.balance, 0),
                      )}
                    </td>
                    <td
                      style={{
