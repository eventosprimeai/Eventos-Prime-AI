                ...
                selectedTask && (
                    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.8)", backdropFilter: "blur(4px)", display: "flex", justifyContent: "flex-end", zIndex: 9999, animation: "fadeIn 0.2s ease" }}>
                        <div style={{ width: "100%", maxWidth: 750, background: "var(--color-bg-primary)", height: "100vh", display: "flex", flexDirection: "column", borderLeft: "1px solid var(--color-border)", boxShadow: "-4px 0 24px rgba(0,0,0,0.5)" }}>
                            ...
                            {/* Chat Messages */}
                            <div style={{ flex: "1 1 auto", minHeight: 0, padding: "var(--space-4)", overflowY: "auto", display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
                                {messages.length === 0 ? (
                                    ...
                                    messages.map(msg => {
                                        ...
                                        return (
                                            <div key={msg.id} style={{ display: "flex", gap: "var(--space-2)", flexDirection: isMe ? "row-reverse" : "row" }}>
                                                ...
                                                <div style={{ maxWidth: "75%", background: isMe ? "var(--color-bg-elevated)" : "var(--color-bg-input)", padding: "var(--space-3) var(--space-4)", borderRadius: "var(--radius-lg)", border: isMe ? "1px solid var(--color-gold-400)" : "1px solid var(--color-border)", borderTopRightRadius: isMe ? 0 : "var(--radius-lg)", borderTopLeftRadius: !isMe ? 0 : "var(--radius-lg)" }}>
                                                    <div style={{ fontSize: "var(--text-xs)", color: "var(--color-text-muted)", marginBottom: 6, fontWeight: 600 }}>
